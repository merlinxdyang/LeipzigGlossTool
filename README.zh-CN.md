...
输出控件保持横向紧凑排列。点击“调整顺序”后才显示拖动手柄；预览、Word/HTML 复制、SVG、PNG、排版设置表和项目文件统一使用该顺序。键盘用户可用左右方向键移动。
...
用户输入的空格始终是最终词界。维吾尔文按逻辑顺序保存；维吾尔语和自定义语言“从右到左”配置只让输入框与原文词项使用 RTL，例句列、编号、编辑卡片和全部导出仍按 LTR 从左到右排列。输入中的 RLO/LRO 等方向控制符会被阻止。
