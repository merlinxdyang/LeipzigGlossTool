...
工具可透過 DeepSeek、OpenAI、Claude 或 OpenRouter 產生相互對齊的原文、轉寫、帶調號拼音、Gloss 和英文自由翻譯。所有結果皆可逐格修改，並可複製到 Word，或匯出為 SVG 和透明 PNG。
...
- 支援國語、粵語和自訂漢語方言。
- 轉寫 1 可選擇拼音、粵拼、注音符號、IPA、耶魯拼音或自訂系統。
- 轉寫 2 固定為帶調號的標準漢語拼音；輕聲不標 `0`，也不加調號。
- 每個詞的原文、兩層轉寫和 Gloss 均可編輯，英文翻譯亦可修改。
- 輸出列可任意組合，並可使用數字編號、`(a)` 編號、`a.` 編號或不編號。
- 每列均可分別設定字型、字級、粗體和斜體；全大寫語法標記會自動使用小型大寫字母。
